...
export class VerticalTextTemplate implements IAnimationTemplate {
  ...
  renderCharContainer(
    ...
    // アルファベット回転判定
    const isAlphabet = textDirection === 'vertical' && 
                      params.enableAlphabetRotation && 
                      /[a-zA-Z0-9]/.test(text);
    ...
    // アルファベット回転を適用
    if (isAlphabet) {
      textObj.rotation = (90 * Math.PI) / 180; // 90度回転
      console.log(`[VerticalTextTemplate] アルファベット回転適用: "${text}"`);
    }
    
    // 回転が指定されている場合は適用（デバッグ用）
    if (params.rotation) {
