...
export class BlinkFadeTextPrimitive_v2 implements IAnimationTemplate {
  ...
  renderWordContainer(
    ...
    // PurePrimitiveWordSlideText互換: 単語コンテナは常に(0, 0)に配置
    // FlexibleCumulativeLayoutPrimitiveが文字の実際の配置を管理する
    container.position.set(0, 0);
    container.visible = true;
  ...
  renderCharContainer(
    ...
    if (!(container as any)[stateKey]) {
      if (forcePhraseTiming) {
        ...
        if (!(container as any)[logKey]) {
          console.log(`[BlinkFadeTextPrimitive_v2] FIXED Animation state char[${charIndex}] "${text}":`);
          console.log(`  flickerStart: ${entranceStartMs} (transformed), duration: ${entranceEndMs - entranceStartMs}, complete: ${entranceEndMs}`);
          console.log(`  nowMs: ${nowMs}, inPhase: ${nowMs >= entranceStartMs && nowMs < entranceEndMs}, headTime: ${headTime}`);
          console.log(`  Using transformed timing: startMs=${startMs}, endMs=${endMs}`);
    ...
    if (nowMs < charState.flickerStartTime) {
      ...
      // 簡潔なデバッグ情報（最初の文字のみ）
      const flickerDebugKey = `__flickerDebugLogged_${charIndex}_${Math.floor(nowMs / 500) * 500}`;
      if (!(container as any)[flickerDebugKey] && charIndex === 0) {
        console.log(`[BlinkFadeTextPrimitive_v2] Char[${charIndex}] progress=${progress.toFixed(2)}, isInEntrance=${isInEntrancePhase}, baseAlpha=${baseAlpha.toFixed(2)}`);
        (container as any)[flickerDebugKey] = true;
      }
