...
function createTemplateRegistry(): TemplateRegistryEntry[] {
  ...
  console.log('[TemplateRegistry] Creating template registry, config:', config);
  ...
  const entries = config.templates.map(templateConfig => {
    ...
    console.log(`[TemplateRegistry] Loading template: ${templateConfig.id} (${templateConfig.exportName}) - ${TemplateClass ? 'SUCCESS' : 'FAILED'}`);
  ...
  console.log(`[TemplateRegistry] Loaded ${entries.length} templates`);
  return entries;
}
