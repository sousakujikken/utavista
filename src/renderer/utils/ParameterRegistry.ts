...
interface ParameterDefinition {
  ...
  min?: number;
  max?: number;
  options?: string[]; // 選択肢（string型パラメータ用）
  description: string;
}
...
export class ParameterRegistry {
  ...
  registerParameter(definition: ParameterDefinition): void {
    if (this.parameters.has(definition.name)) {
      console.warn(`Parameter ${definition.name} is already registered`);
      return;
    }
    this.parameters.set(definition.name, definition);
  }
   ...
   * パラメータ定義の取得
   */
  getDefinition(name: string): ParameterDefinition | undefined {
   ...
   * パラメータの選択肢を取得
   */
  getParameterOptions(name: string): string[] | undefined {
    const definition = this.parameters.get(name);
  ...
  validateParameter(name: string, value: any): { valid: boolean; error?: string } {
    ...
    if (typeof value !== definition.type) {
      return { 
        valid: false, 
        error: `Invalid type for ${name}: expected ${definition.type}, got ${typeof value}` 
      };
    }
