...
export class SlideAnimationPrimitive implements AnimationPrimitive {
  ...
  executeWithinHierarchy(
    ...
    try {
      ...
      const slideParams: SlideAnimationParams = {
        ...
        initialOffset: params.rightOffset as number || 100,
  ...
  calculatePhrasePosition(params: {
    ...
    if (app && app.renderer) {
      centerX = app.renderer.width / 2 + params.phraseOffsetX;
      centerY = app.renderer.height / 2 + params.phraseOffsetY;
    }
  ...
  calculateWordPosition(params: {
    ...
    let startPositionX = params.rightOffset;
    
    if (params.wordDisplayMode === 'individual_word_entrance_same_line' || 
        params.wordDisplayMode === 'phrase_cumulative_same_line') {
      // same_lineモード: 前の単語の後に配置
      if (params.wordIndex > 0 && params.phraseContainer) {
        // WordContainerAttributeManagerを使用してフレーズコンテナから累積幅を取得
        const cumulativeWidth = WordContainerAttributeManager.calculateCumulativeWidth(
          params.phraseContainer,
          params.wordIndex,  // targetWordIndex: 現在の単語より前の単語を計算
          params.wordSpacing || 1.0,
          params.fontSize,
          params.charSpacing || 1.0
        );
