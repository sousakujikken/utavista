...
const ContentTab: React.FC<ContentTabProps> = ({ engine, onLyricsEditModeToggle }) => {
  ...
  const handleAspectRatioChange = (value: string) => {
    ...
    if (engine) {
      engine.updateStageConfig(aspectRatio, orientation);
  ...
  useEffect(() => {
    const loadRecentFiles = async () => {
      try {
        ...
        const videoFiles = await electronMediaManager.getRecentFiles('video');
          ...
          <Select 
            ...
            onChange={async (e) => {
              if (e.target.value && engine) {
                ...
                if (selectedFile) {
                  try {
                    const result = await electronMediaManager.loadRecentAudio(selectedFile.filePath);
                    if (result) {
                      const { audio, fileName } = result;
                      engine.loadAudioElement(audio, fileName);
                      setMusicFileName(fileName);
            ...
            <Button 
              ...
              onClick={async () => {
                try {
                  ...
                  if (result && engine) {
                    const { videoUrl, fileName } = result;
                    setBackgroundVideoUrl(videoUrl);
                    updateBackgroundConfig({ videoFilePath: videoUrl });
                    
                    setTimeout(async () => {
                      const updatedFiles = await electronMediaManager.getRecentFiles('video');
